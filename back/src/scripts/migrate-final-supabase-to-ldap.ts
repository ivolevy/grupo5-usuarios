/**
 * MIGRACIÓN FINAL: Supabase → LDAP
 * Migra TODAS las columnas de Supabase usando la estrategia que SÍ funciona
 */
...
async function migrateFinalSupabaseToLDAP(): Promise<void> {
  console.log('🚀 MIGRACIÓN FINAL Supabase → LDAP...');
  console.log('📋 Migrando TODAS las columnas usando estrategia probada');
  ...
  try {
    ...
    // Migrar cada usuario usando la estrategia que SÍ funciona
    for (const user of supabaseUsers) {
      try {
        ...
        // Crear atributos usando la estrategia que SÍ funciona
        const userAttributes = {
          ...
          // Mapear TODAS las columnas de Supabase usando atributos estándar
          description: ultraCleanValue(`ID:${user.id.substring(0,8)}|Rol:${user.rol}|Ver:${user.email_verified}|Nac:${user.nacionalidad || 'N/A'}|Tel:${user.telefono || 'N/A'}`),
          displayName: ultraCleanValue(user.nombre_completo || nameData.cn),
          title: ultraCleanValue(user.nacionalidad || 'Usuario'),
          telephoneNumber: ultraCleanValue(user.telefono),
          postalAddress: ultraCleanValue(`Created:${user.created_at?.substring(0,10)}|Updated:${user.updated_at?.substring(0,10)}`),
          businessCategory: ultraCleanValue(`LastLogin:${user.last_login_at?.substring(0,10) || 'N/A'}`),
          seeAlso: ultraCleanValue(`ResetToken:${user.password_reset_token || 'N/A'}|ResetExp:${user.password_reset_expires?.substring(0,10) || 'N/A'}`)
        };
    ...
    migratedUsers.forEach(user => {
      ...
      console.log(`    📝 Info completa: ${user.description}`);
    });

    console.log('\n🎉 MIGRACIÓN FINAL EXITOSA!');
    console.log('📋 TODAS las columnas de Supabase migradas a LDAP:');
    console.log('   ✅ id → description (ID:...)');
    console.log('   ✅ email → mail');
    console.log('   ✅ password → userPassword');
    console.log('   ✅ rol → description (Rol:...)');
    console.log('   ✅ created_at → postalAddress (Created:...)');
    console.log('   ✅ updated_at → postalAddress (Updated:...)');
    console.log('   ✅ email_verified → description (Ver:...)');
    console.log('   ✅ email_verification_token → seeAlso (si existe)');
    console.log('   ✅ password_reset_token → seeAlso (ResetToken:...)');
    console.log('   ✅ password_reset_expires → seeAlso (ResetExp:...)');
    console.log('   ✅ last_login_at → businessCategory (LastLogin:...)');
...
if (require.main === module) {
  migrateFinalSupabaseToLDAP().catch(console.error);
}

export { migrateFinalSupabaseToLDAP };
