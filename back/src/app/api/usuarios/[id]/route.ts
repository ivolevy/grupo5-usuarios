...
export async function PUT(
  ...
  try {
    ...
    console.log('🔍 [UPDATE USER] Buscando usuario en LDAP...');
    let existingUserResult;
    try {
      console.log('🔍 [UPDATE USER] Intentando buscar por UID:', id);
      existingUserResult = await ldapService.getUserByUid(id);
      console.log('🔍 [UPDATE USER] Resultado búsqueda por UID:', existingUserResult.success);
    } catch (error) {
      console.log('🔍 [UPDATE USER] Error en búsqueda por UID, intentando por email:', error);
      existingUserResult = await ldapService.getUserByEmail(id);
      console.log('🔍 [UPDATE USER] Resultado búsqueda por email:', existingUserResult.success);
    }

    if (!existingUserResult.success || !existingUserResult.data) {
      console.log('❌ [UPDATE USER] Usuario no encontrado en LDAP');
      return NextResponse.json({
        success: false,
        message: 'Usuario no encontrado en LDAP'
      }, { status: 404 });
    ...
    console.log('✅ [UPDATE USER] Usuario encontrado:', existingUserResult.data.uid);
    ...
    console.log('🔧 [UPDATE USER] Datos a actualizar:', updateData);

    // Actualizar el usuario en LDAP
    const updateResult = await ldapService.updateUser(existingUser.uid, updateData);

    if (!updateResult.success) {
      console.log('❌ [UPDATE USER] Error al actualizar en LDAP:', updateResult.error);
      return NextResponse.json({
        success: false,
    ...
    console.log('✅ [UPDATE USER] Usuario actualizado exitosamente en LDAP');
